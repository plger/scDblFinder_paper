...
'Heterotypic' doublets (formed by cells of distinct transcriptional states), instead, can appear as an artefactual novel cell type and disrupt downstream analyses [@germainPipeCompGeneral2020b].
...
Indeed, because artificial doublet creation can only approximate real doublets, a risk of classifier-based approaches is that the exact classification problem on which they are trained (distinguishing _artificial_ doublets from real cells) differs slightly from the real problem on which they are expected to function (distinguishing _real_ doublets from singlets).
...
We defined two forms of doublet enrichment (Figure \@ref(fig:dblenrScheme)), and specified models to test each possibility:
