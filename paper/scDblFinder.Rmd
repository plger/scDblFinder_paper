...
Building on the strengths of each of these different approaches, we developed `scDblFinder`, 
a Bioconductor-based, fast, flexible and accurate doublet detection method.
`scDblFinder` was already found by and independent benchmark to outcompete alternatives.
Here we present the method, justify its design choices, demonstrate its superiority on both single-cell RNAseq and single-cell ATACseq data, and provide some observations on doublet formation and detection.
...
It is therefore common in contemporary single-cell experiments to have ~10% doublets (sometimes even ~20%), making adequate doublet detection critical.
...
Although often larger, median doublets library sizes were systematically smaller than the sum of the median library sizes of composing cell types (Figure \@ref(fig:realDbls)A).
...
```{r realDbls, fig.width=9, fig.height=8, fig.cap="Characterization of real doublets. A: Observed median (and median absolute deviation in) library sizes per cell type against additive expectation for single cell and doublet types in a real dataset. B: Relative contribution of composing cell types in real doublets (each point represents a doublet) plotted against the expected relative contributions."}
...
Although differences in median library size across cell types were small (less than two-fold) compared to other datasets, we observed a weak associated of the relative contributions with the relative sizes of the composing cell types (Figure \@ref(fig:realDbls)B, p~`r format(coef(summary(lm(ratio~0+expected.ratio, data=ag2)))[1,4], digits=2)`).
This effect was however considerably smaller than the variation within type.
This suggests that 
i) large variations in real cell size within a given cell type, and/or
ii) large variations in the mRNA sampling efficiency which are independent for the two composing cells.
...
In doing so, we reduce the necessary number of artificial doublets (since no artificial doublet is 'lost' modeling homotypic doublets), and prevent the classifier from being train to recognize cells that and undistinguishable from singlets.
Standard dimensional reduction is then performed on the union of real cells and artificial doublets, and a nearest neighbor network is generated.
...
To those are added further cell-wise predictors, such as library size, co-expression scores [based on a variation to @baisScdsComputationalAnnotation2020], ambiguity of marker signature (Spearman correlation of cluster markers with cluster averages), etc.
...
We optimized a number of parameters in the procedure, notably regarding the generation of artificial doublets, learning hyper-parameters, and features to include, so as to provide robust default parameters.
...
Some features, such as the distance to the nearest doublet or whether the nearest neighbor is an artificial doublet, had a negative impact on performance (Supplementary Figure 2A), presumably because it led to over-fitting.
...
This resulted in a loss of accuracy in real datasets (Supplementary Figure 2B).

We finally optimized training hyper-parameters (Supplementary Figure 3A) as well as the number of iterations (Supplementary Figure 3B), finding that one or two additional training iterations were sufficient.
...
Adjusting for both types of error in the truth, the area under the PR curve is considerably better (0.82 instead of 0.64), and we estimate that ~90\% of heterotypic doublets can be identified with a real FDR of ~0.2 (a similar analysis for a different sample is show in Supplementary Figure 4).
...
Unfortunately, prediction based on nearest neighbors proved inaccurate, both in real and simulated data (Figure \@ref(fig:dblTypes)).
...
sce <- scDblFinder(mockDoubletSCE(c(300,500,500,800), ngenes=1000))
w <- which(sce$type=="doublet" & sce$scDblFinder.class=="doublet")
h2 <- Heatmap(
  unclass(table(sce$scDblFinder.mostLikelyOrigin[w], gsub("cluster","",sce$origin[w]))), 
  row_title="Predicted type", column_title="True type", name="# doublets",
  cluster_columns=FALSE, cluster_rows=FALSE, col=viridisLite::cividis(20))
...
Assuming the doublet characterization was adequate, we could then investigate whether more doublets of given types were found then expected.
...
All methods however failed to appropriately control FDR.
