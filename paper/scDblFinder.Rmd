...
`scDblFinder` has the highest mean area under the precision-recall (PR) curve (Supplementary Figure 6), ranking first in a majority of datasets, and otherwise typically very close to the top.
...
Adjusting for both types of error in the truth, the area under the PR curve is considerably better (0.82 instead of 0.64), and at the automatic threshold we estimate that `r round(d$y/(1-prop.homotypic)*100)`\% of heterotypic doublets can be identified with a real FDR of `r round(100*d$x-prop.intraind)`\% (a similar analysis for a different sample is shown in Supplementary Figure 8).
...
With default parameters, `scDblFinder` performed very poorly (Supplementary Figure 10).
...
This method yielded equal or better performance than specialized single-cell ATACseq software (Supplementary Figure 10).
