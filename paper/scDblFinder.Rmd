...
p2 <- ggplot(dbls, aes(expectedProp, prop.type1)) + geom_abline(linetype="dashed", col="grey") +
  ...
  labs(x="Expected proportion of first celltype (based on median celltype library size)",
       y="Estimated proportion of first celltype", size="Library size")
...
If the cluster-based approach is used, the top features per cluster are instead selected.
If `clusters` are not given, PCA (using the `scater` package with the Irlba approximation) and clustering (using `scDblFinder`'s `fastcluster` function) are performed.
The `fastcluster` function generates a kNN network (using `BiocSingular` with the Annoy approach) followed by louvain clustering.
...
Artificial doublets are then created by combining cells of different clusters in a proportional fashion. In explicitly concentrating on inter-cluster doublets, we do not attempt to identify homotypic doublets, which are anyway virtually unidentifiable and relatively innocuous.
...
In practice, both approaches have comparable performances (see below), and they can also be combined.
...
Some features, such as the distance to the nearest doublet or whether the nearest neighbor is an artificial doublet, had a negative impact on performance (Supplementary Figure 1), presumably because it led to over-fitting.
...
This resulted in a reduction in AUPRC in real datasets (Supplementary Figure 2 and Figure \@ref(fig:benchmark1)).

We finally optimized training hyper-parameters (Supplementary Figure 3) as well as the number of iterations (Supplementary Figure 4), finding that a relatively low number of iterations (2-3) was sufficient.
...
`scDblFinder` has the highest mean area under the precision-recall (PR) curve (Supplementary Figure 5), ranking first in a majority of datasets, and otherwise typically very close to the first.
...
e$point.colour <- viridisLite::cividis(100)[round(100*e$AUPRC)]
e$border.colour <- ifelse(e$AUPRC==datmax[e$dataset], "black", NA)
...
scdbl.methods <- c("scDblFinder","scDblFinder.rnd","directDblClassification","computeDoubletDensity")
...
Adjusting for both types of error in the truth, the area under the PR curve is considerably better (0.82 instead of 0.64), and we estimate that ~90\% of heterotypic doublets can be identified with a real FDR of ~0.2 (a similar analysis for a different sample is shown in Supplementary Figure 6).
...
To investigate this, we implemented different multi-sample approaches and tested them on two real multi-sample datasets with demuxlet-based true doublets, as well as a sub-sampling of them (Supplementary Figure 7).
...
```{r scATAC, fig.height=4, fig.width=6, fig.cap="Performance of scDblFinder with default (.raw) parameters or on aggregated features (.aggregation) versus ArchR."}
...
The `stickiness' of each cluster (as proxy for cell types) can be evaluated by fitting a single generalized linear model on the observed abundance of doublets of each origin, in the following way:

$$
log(\text{observed}_i+0.1) = log(\text{expected}_i) + \beta_z \cdot log(\text{difficulty}_i) +
\beta_a a_i + \beta_b b_i + \beta_c c_i + ... +\epsilon_i ,
$$
where $observed_i$ and $expected_i$ represent the numbers of doublets formed by specific combination $i$ of clusters which are respectively observed or expected from random combinations, and $a_i$, $b_i$ and $c_i$ (etc) indicate whether or not (0/1) the doublet involves each cluster.
...
We tested the performance of the different underlying distributions using simulations, where the actual number of doublets of each type is generated from expectation with or without added stickiness (as factors of 1 to 3 on the probability) using negative binomial distributions with different over-dispersion parameters Figure \@ref(fig:enrFDR)A).
...
This was robust across different over-dispersion values (Supplementary Figure 9).
...
p1 <- plotROCs(lapply(scores, FUN=function(x) 1-x$FDR), scores[[2]]$truth, th=0.95, size=4, fdr=TRUE) + scale_colour_manual(values=cols)
...
```{r enrFDR, fig.width=8, fig.height=4, fig.cap="Performance of the cluster stickiness test using different underlying distributions."}
plot_grid(p1 + ggtitle("Cluster stickiness") + theme(legend.position="none"), 
          p2 + ggtitle("Specific combinations"), leg,
          nrow=1, rel_widths=c(3,3,1), scale=0.95, labels=c("A","B",""))
```
...
We again tested this approach relying on different underlying distributions, on simulations with varying over-dispersion (Figure \@ref(fig:enrFDR)B). The negative binomial performed better, however all methods failed to appropriately control FDR.
