...
suppFig <- function(increment=NULL, nb=NULL){
  ...
  if(!increment){
    suppFigN <- suppFigN+as.integer(increment)
...
Interestingly, despite several new publications, the initial benchmark identified the oldest method, `DoubletFinder` [@mcginnisDoubletFinderDoubletDetection2019].
...
pdf("Figure3.pdf", width=9, height=8)
...
All other things being equal, this strategy did not lead to a clear overall improvement across the datasets (`r suppFig()`) over the simple sum (both of which were clearly superior to averaging), suggesting that most of the difference is anyway within the wide variability in library sizes, and that the normalization and dimensionality reduction steps are sufficient to remove remaining differences between real and artificial doublets. Another possible interpretation is that doublets can be approximated as the sum of the counts of the composing cells, but that doublets show a bias towards smaller cells. Either way, we nevertheless maintained the mixed doublet generation strategy by default, as it was not deleterious and might prove more robust across more varied protocols.
...
We found that 2-3 iterations provided the best performance (`r suppFig()`).
...
`scDblFinder` has the highest mean area under the precision-recall (PR) curve (see `r suppFig()`), ranking first in a majority of datasets, and otherwise typically very close to the top.
In addition, `scDblFinder` runs at a fraction of the time required by the next best methods (Figure \@ref(fig:benchmark1), left).

```{r benchmark1, fig.width=9, fig.height=4.6, fig.cap="**Benchmark.** Accuracy (area under the precision and recall curve) of doublet identification using alternative methods across 16 benchmark datasets. The size of the dots indicate the relative ranking for the dataset, and the numbers indicate the actual area under the (PR) curve. For each dataset, the top method is circled in black. Methods in bold are available through the scDblFinder package."}
...
pdf("Figure4.pdf", width=9, height=4.6)
...
pdf("Figure5.pdf", width=9, height=4)
...
(not adjusting, however, for the base rate of doublets).
...
This is for instance the strategy used by @wolockScrubletComputationalIdentification2019, which uses the best tradeoff in misclassifying artificial doublets from real droplets.
...
`scDblFinder` includes a thresholding method that combines both rationales, and attempts to minimize both the proportion of artificial doublets being misclassified and the deviation from the expected doublet rate (see [Thresholding]).
...
```{r thresholding, fig.height=8, fig.width=9, eval=TRUE, warning=FALSE, fig.cap="**Thresholding. A-B:** ROC curves (with square-root transformation on the x axis) of the different benchmark datasets. In **A**, the dots indicate the point of greatest distance to the diagonal (elbow). In B, the crosses indicate the thresholds established through the combined thresholding method, and the colors indicate the scDblFinder doublet scores. **C:** Deviation from the elbow of different thresholding procedures, expressed as a proportion of the number of droplets."}
...
plotROCscores <- function(rocs, legend=FALSE, addRandom=TRUE, rasterize=FALSE){
  w <- which(!rocs$called & !duplicated(rocs[,c("Dataset","called")]))
  rot <- rocs[w-1,]
  p <- ggplot(rocs, aes(FPR, TPR, colour=score)) + scale_x_sqrt() + 
    geom_line(colour="grey", aes(group=Dataset))
  if(rasterize){
    p <- p + ggrastr::geom_point_rast(size=0.8)
  }else{
    p <- p + ggrastr::geom_point(size=0.8)
  }
...
#   x <- x[order(-x$score),]
#   randomFDR <- 1-table(x$truth)[2]/nrow(x)
#   #x <- x[x$FDR<=(2*randomFDR),]
#   x$ddist <- x$TPR-x$FDR
#   farthestPoint <- which.max(x$ddist)
#   farthestDist <- x$ddist[farthestPoint]
#   s50 <- head(which(x$score<=0.5),1)
...
#                  "# expected\nheterotypic"=expected2, "combined\nthreshold"=th),
#                FUN=function(i){
#     c(FDR=x$FDR[farthestPoint], TPR=x$TPR[farthestPoint], thFDR=x$FDR[i], thTPR=x$TPR[i], 
#       distDev=x$ddist[i]-farthestDist,
#       nbDev=i-farthestPoint,
#       prop.dev=(i-farthestPoint)/nrow(x))
#   })
...
p2 <- plotROCscores(rocs, TRUE, rasterize=TRUE) + 
  ggtitle("scDblFinder combined thresholds")

p3 <- ggplot(dr, aes(method, (prop.dev), fill=method)) + 
  geom_hline(yintercept=0, linetype="dashed") + geom_violin() + 
  geom_boxplot(width=0.1, show.legend=FALSE) + 
  labs(x="Thresholding", y="Proportion deviation from elbow") +
  theme(legend.key.height=unit(2, 'lines')) +
  ggtitle("Deviation of computed thresholds from the ROC elbow")
...
ths <- dplyr::bind_rows(lapply(ds, FUN=function(x){
  ...
  ths <- list(expected=expected, expected.heterotypic=expected2,
              score50=sum(x$scDblFinder.score>0.5),
...
ggplot(ths, aes(FDR, TPR, colour=method)) + geom_point() + facet_wrap(~dataset)
...
p <- plot_grid(
  ...
  p3, labels=c("A","B",NA,"C"), nrow=2)



pdf("Figure6.pdf", width=9, height=8)
p
dev.off()
p
```
...
pdf("Figure7.pdf", width=8, height=4)
p
dev.off()
p
```
...
We next investigated whether `scDblFinder` could be applied to other types of single-cell data prone to doublets, such as single-cell ATACseq (scATACseQ). We compared scDblFinder to two methods specifically designed to scATACseq: the `ArchR` package [@granjaArchRScalableSoftware2021], which implements a doublet detection method that is also based on the comparison to artificial doublets, and the `Amulet` method [@thibodeauAMULETNovelRead2021]. `Amulet` is based on the assumption that, in a diploid cell, any given genomic region should be captured at most twice, and therefore interprets a larger number of loci with more than two reads as indicative of the droplet being a doublet. Since it was only available in the form of a mixture of java and python scripts, we re-implemented the method in `scDblFinder`, leading to equal or superior results (Figure \@ref(fig:scATAC)). Of note, the `Amulet` method has the advantage of capturing homotypic doublets, which instead tend to be missed by other methods.

The methods were compared across three datasets with a genotype-based annotation (used as ground truth): two obtained from @granjaArchRScalableSoftware2021, which by design do not have homotypic doublets, and the dataset published along `Amulet` (GSM5457171; @thibodeauAMULETNovelRead2021). The latter contains homotypic doublets, but its doublet annotation is highly incomplete: due to the low multiplexing, we expected to have approximately 35\% of the doublets mislabeled as singlets.
...
```{r scATAC, fig.height=3.5, fig.width=7, fig.cap="**Doublet identification in single-nucleus ATAC-seq.** 'amulet.py' and 'amulet.R' respectively stand for the original and R reimplementation of the method. 'scDblFinder.agg' stands for the feature aggregation approach. 'combination' indicates a Fisher combination of the amulet.R p-value and the 1 minus the scDblFinder.agg score. For 'ArchR', the DoubletEnrichment output was used."}
ov <- readRDS("../other_datasets/atac.results.rds")
ld <- split(ov, ov$dataset)
stats <- lapply(setNames(names(ld),names(ld)), FUN=function(x){
  x <- ld[[x]]
  scores <- intersect(c("amulet.py", "amulet.R", "scDblFinder.raw","scDblFinder.agg", "DoubletEnrichment",
                        "comb1"), colnames(x))
  x <- x[!is.na(x$truth),]
...
s2$method <- factor(s2$method, rev(c("DoubletEnrichment","amulet.py","amulet.R","scDblFinder.raw", "scDblFinder.agg", "comb1")))
levels(s2$method)[1] <- "combination"
...
ggplot(s2, aes(method, value, fill=metric)) + geom_col(position=position_dodge()) + facet_wrap(~Dataset) + coord_flip()

pdf("Figure8.pdf", width=7, height=3.5)
p
dev.off()
p
```

While of an elegant simplicity, the `Amulet` approach performed well only on one dataset (Figure \@ref(fig:scATAC)). Of note, however, the authors indicate that larger library sizes are needed for the approach to perform well, which is not the case for most cells in these datasets. Another problem is that the number of loci with more than two reads is strongly dependent on library size (`r suppFig()`), however this dependency cannot easily be taken into account because doublets also tend to have a larger library size, making the two variables confounded.

These results show that the `scDblFinder` approach can be applied to scATACseq data, although more work and a broader set of benchmark datasets will be necessary to establish optimal methods. In the meantime, we recommend using more than one method: for example, a simple Fisher *p*-value aggregation between the `Amulet` and `scDblFinder` (aggregation) methods provides a performance that is locally suboptimal but considerably more robust across datasets. Other approaches provide more complex ways of aggregating calls from different methods [@xiongChordIdentifyingDoublets2021; @neavinDemuxafyImprovementDroplet2022].

## Doublet origins and enrichment analysis

When artificial doublets are generated between clusters, we can keep track of the clusters composing them, and we reasoned that this information could be used to infer the clusters composing real doublets (hereafter referred to as 'doublet origin'). Using a simulation as well as the aforementioned real dataset with doublets of known origins (mixture of five cell lines from @tianScRNAseqMixologyBetter2018), we assessed the accuracy of doublet origin prediction based on the nearest artificial doublets in the kNN. These proved inaccurate, both in real and simulated data (see `r suppFig(FALSE)`A-B). Even training a classifier directly on this problem failed (see `r suppFig(FALSE)`C-D).
...
pdf("Figure9.pdf", width=8, height=6)
p
dev.off()
p
```
...
Further cell-level predictors are added, including: projections on principal components; library size; and co-expression scores [based on a variation of @baisScdsComputationalAnnotation2020].
...
Using the benchmark datasets from @xiBenchmarkingComputationalDoubletDetection2021, we next optimized a number of parameters in the procedure, notably regarding features to include and hyperparameters, so as to provide robust default parameters (see `r suppFig(6)`).
Some features, such as the distance to the nearest doublet or whether the nearest neighbor is an artificial doublet, had a negative impact on performance (see `r suppFig(nb=-5)`), presumably because it led to over-fitting.
Indeed, because artificial doublet creation can only approximate real doublets, a risk of classifier-based approaches is that the exact classification problem on which they are trained, namely distinguishing _artificial_ doublets from _real_ droplets, slightly differs from the real problem on which they are expected to function (distinguishing _real_ doublets from _real_ singlets).
...
This resulted in a reduction in area under the precision and recall curve (AUPRC) in real datasets (see `r suppFig(nb=-4)`; see also Figure \@ref(fig:benchmark1)).
Finally, in line with a discrepancy between the trained and real problems, we observed that the variable importance calculated during training (see `r suppFig(nb=-3)`) did not necessarily match that of the variable drop experiments (see `r suppFig(nb=-5)`).

We finally optimized hyperparameters (see `r suppFig(nb=-2)`) as well as the number of iterations (see `r suppFig(nb=-1)`), finding that a relatively low number of iterations (2-3) was sufficient.
...
where $o_t$ represents the number of real droplets classified as doublets at threshold $t$, and $e_{low}$ and $e_{high}$ represent, respectively, the lower and higher bounds of the expected number of heterotypic doublets in the dataset (based on the given or estimated doublet rate \pm the `dbr.sd` parameter). The default value of the `dbr.sd` parameter was roughly estimated from the observed doublet rates. XXXXXXXXXXXXXXXXXXXXXXX
...
For feature aggregation, `scDblFinder` first normalizes the counts using the Term Frequency - Inverse Document Frequency (TF-IDF) normalization, as implemented in @stuartComprehensiveIntegrationSingleCell2019. PCA is then performed and the features are clustered into the desired number of meta-features using mini-batch k-means [@hicksMbkmeansFastClustering2021] or, if not available, simple k-means. The counts are then summed per meta-feature.
...
CGM developed the conformal amulet approach. 
...
We thank Nan Miles Xi and Jingyi Jessica Li for help with their benchmark datasets; Jeffrey Granja for support with ArchR and its attached datasets; and the Robinson group and users for feedback.
