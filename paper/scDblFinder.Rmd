...
Here we present the method, justify its design choices, demonstrate its performance on both single-cell RNA and accessibility sequencing data, and provide some observations on doublet formation, detection, and enrichment analysis.
...
For example, <a href="https://github.com/chris-mcginnis-ucsf/DoubletFinder">`DoubletFinder`</a> generates a _k_-nearest neighbor (kNN) graph on the union of real cells and artificial doublets, and estimates the density of artificial doublets in the neighborhood of each cell [@mcginnisDoubletFinderDoubletDetection2019].
In a similar fashion, one of the methods proposed by @baisScdsComputationalAnnotation2020, <a href="https://bioconductor.org/packages/release/bioc/html/scds.html">`bcds`</a>, generates artificial doublets and trains a classifier to distinguish them from real cells.
Real cells that are classified with artificial doublets are then called as doublets.
Finally, another strategy proposed by @baisScdsComputationalAnnotation2020 is a coexpression score, <a href="https://bioconductor.org/packages/release/bioc/html/scds.html">`cxds`</a>, which flags cells that co-express a number of genes that otherwise tend to be mutually exclusive across cells.
...
Finally, a thresholding procedure decides the score at which to call a cell by simultaneously minimizing the misclassification rate and the expected doublet rate (see [Thresholding]).
...
Using the benchmark datasets from @xiBenchmarkingComputationalDoubletDetection2021, we next optimized a number of parameters in the procedure, notably regarding features to include and hyperparameters, so as to provide robust default parameters (see Extended Data - Figures 1-6).
Some features, such as the distance to the nearest doublet or whether the nearest neighbor is an artificial doublet, had a negative impact on performance (see Extended Data - Figure 1), presumably because it led to over-fitting.
...
This resulted in a reduction in area under the precision and recall curve (AUPRC) in real datasets (see Extended Data - Figure 2; see also Figure \@ref(fig:benchmark1)).
Finally, in line with a discrepancy between the trained and real problems, we observed that the variable importance calculated during training (see Extended Data - Figure 3) did not necessarily match that of the variable drop experiments (see Extended Data - Figure 1).

We finally optimized hyperparameters (see Extended Data - Figure 4) as well as the number of iterations (see Extended Data - Figure 5), finding that a relatively low number of iterations (2-3) was sufficient.

### Fast clustering

Irlba-based singular value decomposition is first run using the `r Biocpkg("scater")` package, and a kNN network is generated using the Annoy approximation implemented in `r Biocpkg("BiocSingular")`.
Louvain clustering is then used on the graph.
If the dataset is sufficiently large (>1000 cells), a first rapid k-means clustering (using the `r Biocpkg("mbkmeans")` package) is used to generate a large number of meta-cells, which are then clustered using the graph-based approach, propagating clusters back to the cells themselves.
...
The cost function being minimized is then simply given by $cost_t = FNR_t + FPR_t + deviation_t^2$, where the false negative rate ($FNR_t$) represents the proportion of artificial doublets misclassified as singlets at threshold $t$, and the false positive rate ($FPR_t$) represents the proportion of real cells classified as doublets. This is illustrated in Figure 
...
Here we reproduced this benchmark using the most recent versions of the packages, and including variant methods from the `scDblFinder` package (among which the updated version of `r Biocpkg("scran")`'s original method, and now available in the `scDblFinder` package as `computeDoubletDensity`). 
Figure \@ref(fig:benchmark1) compares the performance of `scDblFinder` to alternatives across the real benchmark datasets.
`scDblFinder` has the highest mean area under the precision-recall (PR) curve (see Extended Data - Figure 7), ranking first in a majority of datasets, and otherwise typically very close to the top.
...
pdf("Figure4.pdf", width=9, height=4.5)
...
Adjusting for both types of error in the truth, the area under the PR curve is considerably better (0.82 instead of 0.64), and at the automatic threshold we estimate that `r round(d$y/(1-prop.homotypic)*100)`\% of heterotypic doublets can be identified with a real FDR of `r round(100*d$x-prop.intraind)`\% (a similar analysis for a different sample is shown in Extended Data - Figure 9).
...
p3 <- plotROCscores(getRocs(ds, class="called.dbrOnly"), rasterize=TRUE)
...
```{r scATAC, fig.height=4, fig.width=6, fig.cap="**Doublet identification in single-nucleus ATAC-seq** Performance of `scDblFinder` with default (.raw) parameters or on aggregated features (.aggregation) versus ArchR (GSE162690 dataset)."}
cd <- readRDS("../other_datasets/atac.colData.rds")
p <- plotROCs(list( ArchR=cd$DoubletEnrichment, 
               "scDblFinder\nraw"=cd$scDblFinder.raw, 
               "scDblFinder\naggregation"=cd$scDblFinder.score), 
         truth=cd$DemuxletClassify=="DBL", fdr=TRUE)
pdf("Figure8.pdf", width=6, height=4)
p
dev.off()
p
```
...
The number of doublets of each type is generated from random expectation with or without added stickiness (as factors of 1 to 3 on the probability) using negative binomial distributions with different over-dispersion parameters (Figure \@ref(fig:dblenr)C and Extended Data - Figure 10).
...
This was robust across different over-dispersion values (see Extended Data - Figure 10).
...
We thank Nan Miles Xi and Jingyi Jessica Li for help with their benchmark datasets; Jeffrey Granja for support with ArchR and its attached dataset; and the Robinson group and users for feedback.
