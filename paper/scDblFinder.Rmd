...
`scDblFinder` was already compared to existing alternatives in an independent benchmark (see XXX, the protocol and addendum to the original study, @xiBenchmarkingComputationalDoubletDetection2021 ).
Figure \@ref(fig:benchmark) compares the performance of `scDblFinder` to alternatives across their real datasets.
`scDblFinder` has the highest mean areas under the receiver-operating characteristic (ROC) and precision-recall (PR) curves, ranking first in a majority of datasets, and otherwise typically very close to the first.
In addition, in runs at a fraction of the time required by the next most accurate methods.

```{r benchmark1, fig.width=8.5, fig.height=4.5, fig.cap="Accuracy (area under the precision and recall curve) of doublet identification using alternative methods across 16 benchmark datasets. The size of the dots indicate the relative ranking for the dataset, and the numbers indicate the actual area under the (PR) curve."}
e <- read.csv("data/results_alternatives.csv")
res <- readRDS("data/results2.rds")
res$method <- "scDblFinder"

# res2 <- readRDS("data/results_agg.rds")
# row.names(res2) <- res2$dataset
# res2b <- res
# res2b[,c("AUROC","AUPRC")] <- res2[as.character(res2b$dataset),c("AUROC","AUPRC")]
# res2b$method <- "scDblFinder-agg"
# res <- rbind(res, res2b)

e <- rbind(e,res[,colnames(e)])
e <- e[e$method!="doubletCells",]

elap <- as.data.frame(
  rbind(matrix(strsplit("doubletCells Scrublet cxds bcds hybrid DoubletDetection DoubletFinder solo DoubletDecon 37 64 5 46 47 380 243 618 903", " ")[[1]], nrow=9),
        c("scDblFinder",48))) #round(mean(res$elapsed)))))
colnames(elap) <- c("method","time")
row.names(elap) <- elap$method
elap$time <- as.numeric(elap$time)
...
e$method <- factor(e$method, levels=names(sort(rowsum(e$AUROC, e$method, na.rm=TRUE)[,1])))
...
p1 <- ggplot(e, aes(dataset, method, colour=AUPRC, size=AUPRC.rank)) + geom_point() +
  scale_color_viridis_c() + scale_size(range=c(4,11), breaks=c(1, 4, 8), labels=c("worst","median","best")) +
...
elap$method <- factor(elap$method, levels(e$method))
elap <- elap[order(elap$method, na.last=NA),]
p2 <- ggplot(elap, aes(method, time)) + geom_col(width=0.75, fill="#00204DFF") + 
...
```{r benchmark2, fig.width=8.5, fig.height=6, fig.cap="Accuracy of doublet identification using alternative methods. The heatmaps are coloured by rank in the respective dataset, and the numbers indicate the actual area under the ROC (above) or PR (below) curve."}
auprc <- reshape2::dcast(e, method~dataset, value.var="AUPRC")
auprc <- data.frame(row.names=auprc[,1], auprc[,-1])
pr.ranks <- apply(auprc,2,FUN=getranks)
auroc <- reshape2::dcast(e, method~dataset, value.var="AUROC")
auroc <- data.frame(row.names=auroc[,1], auroc[,-1])
ro.ranks <- apply(auroc,2,FUN=getranks)
dimnames(ro.ranks) <- dimnames(pr.ranks) <- dimnames(auroc)

n <- names(rev(sort(rowMeans(ro.ranks, na.rm=TRUE))))

timea <- function(x, axis=TRUE, ...){
  rowAnnotation("mean\nrunTime"=anno_barplot(x, border=FALSE, ..., axis=axis,
              gp=gpar(fill="darkred")), #axis_param=list(direction="reverse")),
              show_annotation_name=axis, annotation_name_rot=90)
}

auroc <- auroc[n,]
ro.ranks <- ro.ranks[n,]

nbd <- setNames(rowSums(res[,c("TP","FN")]), res$dataset)[names(datmax)]
topa <- HeatmapAnnotation("max AUROC"=datmax, show_legend=FALSE,
                          #"# doublets"=nbd, 
                          col=list(
  "max AUROC"=circlize::colorRamp2(breaks=c(0.5,1), c("white","darkgreen"))))
h1 <- pheatmap(ro.ranks, display_numbers=round(as.matrix(auroc),2), cluster_cols=FALSE, 
         cluster_rows=FALSE, border=NA, color=viridis::viridis(10), row_title="AUROC",
         name="Rank", top_annotation=topa, na_col="white", 
         right_annotation=timea(elap[n,"time"], axis=FALSE), 
         legend_breaks=c(1,8), legend_labels=c("worst","best"))

n <- names(rev(sort(rowMeans(pr.ranks, na.rm=TRUE))))
auprc <- auprc[n,]
pr.ranks <- pr.ranks[n,]
h2 <- pheatmap(pr.ranks, display_numbers=round(as.matrix(auprc),2), cluster_cols=FALSE, 
         cluster_rows=FALSE, border=NA, color=viridis::viridis(10), 
         name="Rank", na_col="white", right_annotation=timea(elap[n,"time"]), 
         row_title="AUPRC")
draw(suppressWarnings(h1 %v% h2), merge_legends=TRUE)
```
...
p <- ggplot(d,aes(x,y)) + geom_segment(data=d2, aes(xend=xend,yend=yend), linetype="dashed") +
...
plot_grid(p + xlim(range(d$x)+c(-0.2,0.2)), p2 + theme(legend.position="none"), 
...
sce <- readRDS("../other_datasets/batch1.SCE.rds")
...
```{r}
getderiv <- function(x){
  model <- smooth.spline(x$FPR, x$TPR)
  predict(model, x=min(x$score[x$called]), deriv=1)$y
}
der <- sapply(getRocs(ds, merge=FALSE), FUN=getderiv)
der.dbr <- sapply(getRocs(ds, class="called.dbrOnly", merge=FALSE), FUN=getderiv)
d <- data.frame(method=rep(c("scDblFinder","Expected #\nheterotypic"),each=length(der)),
                dataset=rep(names(der),2), ROC.derivative=c(der,der.dbr))
ggplot(d, aes(dataset, ROC.derivative, fill=method)) + geom_col(position="dodge") + 
...
plotROCs(list(ArchR=cd$DoubletEnrichment, "scDblFinder\nraw"=cd$scDblFinder.raw, "scDblFinder\naggregation"=cd$scDblFinder.score), truth=cd$DemuxletClassify=="DBL", fdr=TRUE)
```

## Doublet characterization

Because doublets are generated between clusters, we can keep track of their origins, and use this to infer the clusters composing real doublets. Unfortunately, this doesn't work very well (Figure \@ref(fig:dblTypes)) -- neither on real data nor even on simulated doublets -- and even training a classifier directly on the problem fails. The problem seems to be that a doublet that's 80% A and 20% B can look a lot like one that's 80% A and 20% C...
...
Heatmap(unclass(table(call=CDd$scDblFinder.mostLikelyOrigin, truth=CDd$demuxlet.dbl.type)),
