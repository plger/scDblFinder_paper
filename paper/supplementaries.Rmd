...
res <- res[res$method!="mixNoResamp" & res$method!="mixA",]
res$method <- factor(res$method, c("sum","mean","fullResamp","mixB"))
levels(res$method)[3] <- "resampled sum"
levels(res$method)[4] <- "mix"
p1 <- ggplot(res, aes(AUPRC.diff, method)) + geom_vline(xintercept=0, linetype="dashed") + 
  geom_violin(fill="lightgrey") + stat_summary() +
  labs(x="Difference to median AUPRC for the dataset", y="Artificial doublet generation")

res <- readRDS("../analyses/optims_iter.rds")
p2 <- vp(res, aes(iter, AUPRC.diff), fill="lightgrey") +
...
plot_grid(
  plot_grid(p1 + coord_flip(), p2, labels="AUTO", scale=0.95, nrow=1),
...
plot_grid(vp(res, aes(processing, AUPRC.diff), fill="lightgrey") + coord_flip() +
  ...
  vp(res, aes(processing, elapsed), fill="lightgrey") + ylab("Running time (s)") + scale_y_log10(),
...
**Doublets and sites covered by more than two reads. A:** Number of loci covered by more than two reads in annotated singlets and doublets in each dataset. **B:** Relationship between library size and the number of loci covered by more than two reads.
